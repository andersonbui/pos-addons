...
class TestUi(odoo.tests.HttpCase):
    def test_pos_credit_invoice(self):
        ...
        env["ir.module.module"].search(
            [("name", "=", "pos_credit_invoice")], limit=1
        ).state = "installed"
        # without a delay there might be problems on the steps whilst opening a POS
        # caused by a not yet loaded button's action
        self.phantom_js(
            "/web",
            "odoo.__DEBUG__.services['web_tour.tour'].run('tour_pos_credit_invoice', 1000)",
            "odoo.__DEBUG__.services['web_tour.tour'].tours.tour_pos_credit_invoice.ready",
            login="admin",
            timeout=200,
        )
